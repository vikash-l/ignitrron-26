      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          ...
          <div>
            <span
              ...
              className="text-2xl font-black tracking-wider cursor-pointer bg-gradient-to-r from-cyan-400 to-purple-500 bg-clip-text text-transparent hover:brightness-110 transition-all mb-4 inline-block"
            >
              {eventData.name}
            </span>
              ...
              <div className="flex space-x-4">
                {eventData.socials.map((social, index) => (
                  <a
                    ...
                    className="p-2 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-cyan-400 hover:border-cyan-500/40 transition-colors duration-200"
          ...
          {eventData.contact && (
            <div>
              <h3 className="text-white font-bold text-base mb-6 tracking-wide uppercase">Get in Touch</h3>
              <ul className="space-y-4">
                <li className="flex items-start text-sm text-slate-400">
                  <Mail className="h-5 w-5 text-cyan-400 mr-3 flex-shrink-0 mt-0.5" />
                  <a href={`mailto:${eventData.contact.email}`} className="hover:text-cyan-400 transition-colors">
                    {eventData.contact.email}
                  </a>
                </li>
                <li className="flex items-start text-sm text-slate-400">
                  <Phone className="h-5 w-5 text-cyan-400 mr-3 flex-shrink-0 mt-0.5" />
                  <a href={`tel:${eventData.contact.phone}`} className="hover:text-cyan-400 transition-colors">
                    {eventData.contact.phone}
                  </a>
                </li>
                <li className="flex items-start text-sm text-slate-400">
                  <MapPin className="h-5 w-5 text-cyan-400 mr-3 flex-shrink-0 mt-0.5" />
          ...
          <div>
            ...
            <div className="p-4 rounded-lg bg-slate-900/50 border border-slate-900/80">
              <span className="text-xs font-semibold text-purple-400 uppercase tracking-widest block mb-1">
                KPR Institute
              </span>
              <p className="text-slate-300 text-xs font-semibold leading-relaxed mb-2">
        ...
        <div className="border-t border-slate-900 pt-8 flex flex-col md:flex-row items-center justify-between">
          <p className="text-slate-500 text-xs mb-4 md:mb-0">
            &copy; {new Date().getFullYear()} {eventData.name}. All rights reserved.
          </p>
