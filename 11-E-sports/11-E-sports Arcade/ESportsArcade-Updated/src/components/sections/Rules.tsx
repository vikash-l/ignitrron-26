        ...
        >
          {eventData.rules.map((rule, index) => (
            <motion.div key={index} variants={itemVariants}>
              <Card className="flex flex-col p-6 bg-slate-900/30 backdrop-blur-sm border-slate-900 hover:border-slate-800 transition-all duration-300 h-full">
                <div className="flex items-center gap-3.5 mb-3 border-b border-slate-905 pb-3">
                  <span className="flex-shrink-0 font-mono font-bold text-xs text-blue-500 bg-blue-600/10 border border-blue-900/20 rounded-md px-2.5 py-1">
                    {rule.id}
                  </span>
