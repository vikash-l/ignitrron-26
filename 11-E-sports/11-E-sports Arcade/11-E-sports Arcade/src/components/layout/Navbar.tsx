    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-600 font-extrabold text-white text-lg transition-transform duration-200 group-hover:scale-105">
              E
            </div>
          ...
          <div className="hidden lg:flex items-center space-x-6 xl:space-x-8">
            {navLinks.map(link => (
              <button
                ...
                className="text-slate-400 hover:text-white font-medium text-sm transition-colors duration-200 cursor-pointer"
              >
                {link.label}
              </button>
            ))}
          ...
          {sections.registration && eventData.registration && (
            <div className="hidden lg:block">
              <a
                ...
                className="inline-flex items-center justify-center px-5 py-2.5 text-sm font-semibold rounded-lg bg-blue-600 hover:bg-blue-500 text-white shadow-md shadow-blue-600/25 transition-all active:scale-[0.98] cursor-pointer"
              >
            ...
            <div className="px-2 pt-4 pb-6 space-y-2 sm:px-3 flex flex-col items-stretch">
              {navLinks.map(link => (
                <button
                  key={link.id}
                  onClick={() => scrollToSection(link.id)}
                  className="w-full text-left px-3 py-2.5 rounded-md text-base font-semibold text-slate-400 hover:text-white hover:bg-slate-900 transition-colors duration-200 cursor-pointer"
                >
                  {link.label}
                </button>
              ))}
              {sections.registration && eventData.registration && (
                <div className="pt-4 px-3">
                  <a
                    href={eventData.registration.url}
                    className="flex w-full items-center justify-center px-4 py-3 text-base font-semibold rounded-lg bg-blue-600 hover:bg-blue-500 text-white shadow-md transition-all cursor-pointer"
                  >
                    {eventData.registration.label} &nbsp; ↗
                  </a>
                </div>
