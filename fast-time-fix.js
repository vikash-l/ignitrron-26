...
const TIME_CHECK_REGEX = /\b(5:00|5\s*pm|5pm|05:00|17:00)\b/i;
...
function walk(dir) {
  ...
  for (const entry of entries) {
    ...
    if (name === 'node_modules' || name === '.git' || name === '.github' || name === '.next' || name === '.cache' || name === 'dist' || name === 'build' || name === 'deployment') continue;
    ...
    if (entry.isDirectory()) {
      ...
      if (/\.(html|js|ts|tsx|json|css|md)$/i.test(name) && !/\.map$/i.test(name)) {
        try {
          ...
          if (!TIME_CHECK_REGEX.test(text)) continue;
...
console.log('Starting fast source-only time replacement (5 PM -> 4 PM)...');
walk(ROOT_DIR);
console.log(`\n====================================================`);
console.log(`DONE: Modified ${fileCount} source files with ${changeCount} time changes.`);
console.log(`====================================================\n`);
