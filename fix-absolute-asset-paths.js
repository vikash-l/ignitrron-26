    ...
    try {
      ...
      for (const e of entries) {
        ...
        if (e.isDirectory()) {
          ...
          // Replace src="/asset.ext" with src="asset.ext" or src="./asset.ext"
          const updated = content.replace(/(src|href|url)\s*=\s*["']\/([^"']+\.(?:png|jpg|jpeg|svg|gif|webp|mp4|webm|avif))["']/gi, (match, p1, p2) => {
            // Keep absolute URLs or data URIs unchanged
          ...
          if (updated !== content) {
            ...
            fixedCount++;
            console.log(`  ✓ Fixed relative path in ${path.relative(fullP, fp)}`);
          }
        }
      }
    } catch (err) {}
...
console.log('================================================================');
console.log('       FIXING ABSOLUTE ROOT ASSET PATHS ACROSS ALL 28 PROJECTS');
console.log('================================================================\n');
