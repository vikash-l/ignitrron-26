...
export const SectionConfigController: React.FC<SectionConfigControllerProps> = ({
  ...
  const sectionKeys = [
    ...
    { key: 'researchJourney', label: 'Research Journey' },
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className="flex items-center gap-2 px-4 py-3 rounded-full bg-slate-900 border border-slate-800 hover:border-emerald-500/30 text-slate-350 hover:text-white font-bold text-xs uppercase tracking-wider shadow-2xl cursor-pointer hover:scale-[1.03] active:scale-[0.98] transition-all select-none"
        >
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 bottom-0 w-[320px] sm:w-[380px] bg-slate-950 border-l border-slate-900 p-6 shadow-2xl z-50 overflow-y-auto flex flex-col justify-between"
            >
              <div>
                {/* Header */}
                ...
                <div className="space-y-3">
                  {sectionKeys.map(({ key, label }) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-sm font-semibold text-slate-350 text-slate-305">
                          {label}
                        </span>
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold transition-all cursor-pointer select-none ${
                            isVisible
                              ? 'bg-emerald-600/10 border-emerald-900/30 text-emerald-500'
